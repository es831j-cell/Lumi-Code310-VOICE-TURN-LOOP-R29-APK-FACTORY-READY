...
public class MainActivity extends Activity {
    ...
    static final long REPLY_ECHO_GUARD_MS = 240L;
    ...
    void showHome(){
        ...
        listenNow.setEnabled(!conversationMode);
        stopListening.setEnabled(conversationMode);
    ...
    void showTalk(){
        ...
        live.setOnClickListener(v->{ exitTextInputModeForVoice(); startConversationMode(); }); stop.setOnClickListener(v->stopConversationMode());
        ...
        try{
            ...
            if(Build.VERSION.SDK_INT>=26){
                android.media.AudioAttributes attrs=new android.media.AudioAttributes.Builder()
                        ...
                        .setContentType(android.media.AudioAttributes.CONTENT_TYPE_SPEECH)
                        .build();
                assistantAudioFocusRequest=new android.media.AudioFocusRequest.Builder(AudioManager.AUDIOFOCUS_GAIN_TRANSIENT_MAY_DUCK)
    ...
    void cancelRecognizerForSpeechOutput(){
        ...
        continuousRecognizer=null;
        micSuppressUntil=Math.max(micSuppressUntil,System.currentTimeMillis()+80L);
        diag("speech","recognizer released for audio output");
    }
    ...
    void recoverTtsAndRetry(String spokenMessage,String reason){
        ...
        if(activeTtsRetryCount>=1 || spokenMessage.trim().isEmpty()){
        ...
        lumiTtsReady=false;
        diag("speech","tts watchdog rebuilding engine for one retry reason="+reason);
        initSpeechOutput();
    }
    ...
    boolean processRecognizedSpeechText(String raw,boolean salvagedPartial){
        ...
        if(usingOnDeviceRecognizer) preferOnDeviceRecognizerRecovery=true;
            ...
            continuousRecognizer.setRecognitionListener(new android.speech.RecognitionListener(){
                ...
                public void onError(int error){
                    ...
                    if(error==android.speech.SpeechRecognizer.ERROR_NO_MATCH || error==android.speech.SpeechRecognizer.ERROR_SPEECH_TIMEOUT){
                        ...
                        // Code302: Samsung/Google STT can repeatedly signal beginning-of-speech and then
                        // return ERROR_NO_MATCH. Two consecutive audio-seen/no-result sessions indicate
                        // a poisoned recognizer instance, not true silence. Recreate it immediately.
                        if(audioSeenThisSession){
                            // Code310: engine recovery is sticky instead of ping-ponging on every
                            // code-7. A system-engine miss switches immediately to on-device. Once
                            // on-device, allow two genuine audio/no-match failures before falling
                            // back to the system recognizer. Any usable transcript resets the streak.
                            boolean onDeviceAvailable=Build.VERSION.SDK_INT>=31 && android.speech.SpeechRecognizer.isOnDeviceRecognitionAvailable(MainActivity.this);
                            String nextEngine;
                            if(!usingOnDeviceRecognizer && onDeviceAvailable){
                                preferOnDeviceRecognizerRecovery=true;
                                onDeviceAudioNoMatchStreak=0;
                                nextEngine="on-device";
                            }else if(usingOnDeviceRecognizer){
                                onDeviceAudioNoMatchStreak=Math.min(4,onDeviceAudioNoMatchStreak+1);
                                if(onDeviceAudioNoMatchStreak>=2){
                                    preferOnDeviceRecognizerRecovery=false;
                                    onDeviceAudioNoMatchStreak=0;
                                    nextEngine="system";
                                }else{
                            ...
                            diag("speech","audio detected but recognizer returned code="+error+"; replacing recognizer; next engine="+nextEngine+" stickyOnDeviceMisses="+onDeviceAudioNoMatchStreak);
                            traceStage("STT","RECOVERY_REBUILD","audioSeenNoMatchStreak="+noMatchAfterAudioStreak+" • nextEngine="+nextEngine+" • stickyRecovery=true");
                            ...
                            if(conversationMode && activityAlive && !manualListeningStop)
                                conversationHandler.postDelayed(() -> startContinuousListening(),420L);
                            return;
                        }
    ...
    void speakAndContinueInternal(String message,boolean alreadyNaturalized){
        ...
        cancelRecognizerForSpeechOutput();
        requestAssistantAudioFocus("reply");
        String spokenMessage=alreadyNaturalized?message:naturalizeSpokenText(message);
    ...
    String systemWiringSnapshot(){
        ...
        s.append("Möbius visual: welded continuous 3D ribbon • neon glass shader • alien runes=true • GPU rendered • bitmap=false\n\n");
